...
cedata_first_authors <- cedata[lead_author==1]

#now we can make a data table with just the affiliation continents and study continents in for each study
df_leadauthors <- data.table(table(cedata_first_authors[,list(affiliation_continent,study_continent)]))
...
ggplot(data=percentage_author_data_continent_yr_plot[country_continent=="Continent"]) + geom_point(aes(x=year_label,y=value,colour=author_label),size=3)+
  ...
  scale_y_continuous(name="Percentage of studies",breaks=seq(0,100,20))+ 
  scale_x_continuous(name="Publication Year", breaks=seq(2009,2017,1)) + labs(colour="Author Type") +
  theme_bw()
...
testlinearmodelallauthors<-glm(all_local_continent_authors~study_continent, data=cedata_uniq_studies, family=binomial)
summary(testlinearmodelallauthors)
anova(testlinearmodelallauthors,
      update(testlinearmodelallauthors, ~1), # update here produces null model for comparison
      test="Chisq")
